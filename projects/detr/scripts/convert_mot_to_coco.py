...
DATA_PATH = '/data00/home/zhangrufeng1/projects/detectron2/projects/detr/datasets/mot/mot17'
OUT_PATH = os.path.join(DATA_PATH, 'annotations')
SPLITS = ['train_half', 'val_half', 'train', 'test']  # --> half数据均来自train.
HALF_VIDEO = True
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        data_path = os.path.join(DATA_PATH, split if not HALF_VIDEO else 'train')
        ...
        for seq in sorted(seqs):
            ...
            num_images = len([image for image in images if 'jpg' in image])  # 一半一半.
            ...
            for i in range(num_images):
                ...
                image_info = {'file_name': '{}/img1/{:06d}.jpg'.format(seq, i + 1),  # 图片名.
                              'id': image_cnt + i + 1,  # 整个训练集的图片编号.
                              'frame_id': i + 1 - image_range[0],  # 一段视频中的第一帧，编号从1开始.
                              'prev_image_id': image_cnt + i if i > 0 else -1,  # 整个训练集的图片编号.
            ...
            if split != 'test':
                ...
                if CREATE_SPLITTED_ANN and ('half' in split):
                    anns_out = np.array([anns[i] for i in range(anns.shape[0])
                                         ...
                                         int(anns[i][0]) - 1 <= image_range[1]], np.float32)  # 拿到指定图片的anns.
                ...
                for i in range(anns.shape[0]):
                    ...
                    if not ('15' in DATA_PATH):
                        if not (float(anns[i][8]) >= 0.25):  # 可见程度.
                            continue
                        if not (int(anns[i][6]) == 1):  # 是否ignore.
                            continue
