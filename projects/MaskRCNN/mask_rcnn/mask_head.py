...
def mask_rcnn_loss_with_gap(pred_mask_logits, instances, vis_period=0):
    """
    ...
    num_dims = len(pred_mask_logits.shape)
    if num_dims == 4:
        mask_side_len = pred_mask_logits.size(2)
        assert pred_mask_logits.size(2) == pred_mask_logits.size(3), "Mask prediction must be square!"
        assert mask_side_len == 1, "After GAP, Mask prediction must be 1."
        pred_mask_logits = pred_mask_logits.view(total_num_masks, -1)
...
class SemiMaskRCNNConvUpsampleHead(BaseMaskRCNNHead):
    """
