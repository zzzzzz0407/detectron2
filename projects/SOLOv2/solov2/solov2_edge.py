...
class SOLOv2_EDGE(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.flag_edge = cfg.MODEL.SOLOV2.FLAG_EDGE
    ...
    def get_ground_truth(self, gt_instances, mask_feat_size=None):
        ins_label_list, cate_label_list, ins_ind_label_list, grid_order_list, ins_id_list\
            = [], [], [], [], []
        for img_idx in range(len(gt_instances)):
            ...
            ins_id_list.append(cur_ins_id_list)
    ...
    def get_ground_truth_single(self, img_idx, gt_instances, mask_feat_size):
        ...
        gt_ins_ids = gt_labels_raw.new_tensor([_ for _ in range(1, num_gt_ins+1)])
        ins_id_list = []
        ...
        for (lower_bound, upper_bound), stride, num_grid \
            ...
            # append.
            ins_id_label = torch.zeros([num_grid, num_grid], dtype=torch.int64, device=device)
            ...
            output_stride = 4
            label_code = []
            for seg_mask, gt_label, half_h, half_w, gt_id in zip(gt_masks, gt_labels, half_hs, half_ws, gt_ids):
                if seg_mask.sum() == 0:
                    continue
                # mass center
    ...
    def loss(self, cate_preds, kernel_preds, ins_pred, targets):
        ins_label_list, cate_label_list, ins_ind_label_list, grid_order_list, ins_id_list = targets
        # ins
        ...
        # ins
        len_img = len(ins_label_list)
        num_ins_level_first = [[] for _ in range(len(ins_label_list[0]))]
        for ins_labels_level in ins_id_list:
            for i in range(len(ins_labels_level)):
                a = ins_labels_level[i]
                b = (a > 0).sum().item()

                num_ins_level_first[i].append(b)

        #id_labels_img_first = [([ins_ids_level_img.flatten()[ins_ids_level_img.flatten().nonzero()[:, 0]].shape[0]
        #                                  for ins_ids_level_img in ins_ids_level])
        #                       for ins_ids_level in zip(*ins_id_list)]
        #id_labels_img_first = [
        #    id_labels_img[id_labels_img.nonzero()[:, 0]]
        #    for id_labels_img in id_labels_img_first
        #]



        #num_ins_level_first = [([ins_labels_level_img.shape[0] for ins_labels_level_img in ins_labels_level])
        #                       for ins_labels_level in zip(*ins_label_list)]
        a = 0
        for _ in num_ins_level_first:
            a += sum(_)
        ...
        loss_ins = []
        seg_mask_img_first = []
        for input, target, num_ins_per_level in zip(ins_pred_list, ins_labels, num_ins_level_first):
            if input is None:
                continue
            input = torch.sigmoid(input)
            loss_ins.append(dice_loss(input, target))
            seg_mask_img_first.append(input.split(num_ins_per_level, dim=0))
        ...
        assert len(pos_inds) == num_ins.item(), print("{}/{}".format(len(pos_inds), num_ins.item()))
        ...
        """
        # append.
        seg_mask_img_first = [torch.cat([ins_labels_level_img
                                         for ins_labels_level_img in ins_labels_level], 0)
                              for ins_labels_level in zip(*seg_mask_img_first)]

        id_labels_img_first = [torch.cat([ins_ids_level_img.flatten()
                                          for ins_ids_level_img in ins_ids_level], 0)
                               for ins_ids_level in ins_id_list]
        id_labels_img_first = [
            id_labels_img[id_labels_img.nonzero()[:, 0]]
            for id_labels_img in id_labels_img_first
        ]
        ...
        losses_edge = 0.
        num_edge = 0
        num_seg = [len(_) for _ in seg_mask_img_first]
        num_seg = sum(num_seg)
        num_id = [len(_) for _ in id_labels_img_first]
        num_id = sum(num_id)
        assert num_seg == num_ins.item(), '{}, {}, {}, {}'.format(num_seg, num_ins, num_id, a)
        assert num_seg == num_id, '{}, {}, {}'.format(num_seg, num_id, len(pos_inds))
        for seg_mask, ins_ids in zip(seg_mask_img_first, id_labels_img_first):
            # assert len(seg_mask) == len(ins_ids)
            if len(ins_ids) == 0:
                continue

            loss_edge = self.edge_loss(seg_mask, ins_ids)
            if loss_edge == None:
                continue
            num_edge += len(loss_edge)
            losses_edge += loss_edge.sum()

        if num_edge > 0:

            losses_edge /= num_edge
        else:
            losses_edge = 0
        ...
        return {'loss_ins': loss_ins,
                'loss_cate': loss_cate,}
                #'loss_edge': losses_edge}

    @staticmethod
    def edge_loss(pred_masks, cate_labels):
        n_samples = len(cate_labels)
        n_samplessss = len(pred_masks)
        assert n_samples == n_samplessss, '{}/{}'.format(n_samplessss, n_samples)


        seg_masks = pred_masks.reshape(n_samples, -1).float()
        inter_matrix = torch.mm(seg_masks, seg_masks.transpose(1, 0))
        cate_labels_x = cate_labels.expand(n_samples, n_samples)
        label_matrix = (cate_labels_x != cate_labels_x.transpose(1, 0)).float()
        if label_matrix.sum() == 0:  # the same gt.
            return None
        inter_matrix = inter_matrix * label_matrix
        max_inters, matched_indices = inter_matrix.max(1)
        unions = []
        with torch.no_grad():
            for i, j in enumerate(matched_indices):
                union = (pred_masks[i] * pred_masks[j]) > 0.01
                union = union.cpu().numpy()
                bbox = cv2.boundingRect(union.astype(np.uint8))
                area = bbox[2] * bbox[3]
                unions.append(area)
            unions = max_inters.new_tensor(unions)

        loss = max_inters / unions + 0.001
        return loss
