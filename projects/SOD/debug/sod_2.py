...
class SOD_2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.loc_prior = cfg.MODEL.SOD.LOC_PRIOR
        ...
        self.post_processors = PostProcess(cfg.MODEL.SOD.MAX_PER_IMG,
                                           cfg.MODEL.SOD.SCORE_THR)
    ...
    def inference(self, outs, images):
        ...
        results = self.post_processors(outs, image_sizes)
        return results
...
class SODInsHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.num_attention = cfg.MODEL.SOD.NUM_ATTENTION
        ...
        if self.use_base:
            ...
            for i in range(self.num_attention):
                base_att.append(nn.Conv2d(
                    self.num_channels, self.num_kernels, kernel_size=1, stride=1, padding=0
                ))
                ins_att.append(nn.Conv2d(
                    self.num_channels, self.num_kernels, kernel_size=1, stride=1, padding=0
                ))
                refine_tower.append(
                    nn.Sequential(
                        nn.Conv2d(self.num_in_channels, self.num_channels, kernel_size=3,
                                  stride=1, padding=1, bias=self.norm is None),
                        nn.GroupNorm(32, self.num_channels),
                        nn.ReLU(inplace=True),
                    )
                )
            self.base_att = nn.ModuleList(base_att)
            self.ins_att = nn.ModuleList(ins_att)
            self.refine_tower = nn.ModuleList(refine_tower)
        ...
        conv_modules = []
        if self.use_base:
            conv_modules += [self.base_att, self.ins_att, self.refine_tower]
    ...
    def attention_forward(self, ins_features, base_features, num_feats=5, index=0):
        bsz, ch, h, w = ins_features.size()
        # query.
        ins_query = self.ins_att[index](ins_features)
        ins_query = ins_query.flatten(2).permute(0, 2, 1)
        scaling = float(ins_query.shape[2]) ** -0.5
        ins_query = ins_query * scaling
        # key.
        base_key = self.base_att[index](base_features)
    ...
    def forward(self, ins_features, base_features=None):
        ...
        if self.use_base:
            for i in range(self.num_attention):
                ins_attn = self.attention_forward(ins_features, base_features, num_feats, i)
                ins_features = ins_features + ins_attn
                ins_features = self.refine_tower[i](ins_features)
